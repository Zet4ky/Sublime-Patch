...
function Restart-Script {
    try {
            [String]$cmd = "@echo off & chcp 850 >nul && cls && powershell.exe -Executionpolicy Unrestricted -Command `"Invoke-RestMethod https://raw.githubusercontent.com/Zet4ky/Sublime-Patch/main/subl_patch.ps1 | Invoke-Expression`" "
            Write-Host "Script is restarting, wait..."
            Start-Process -FilePath "conhost.exe" -ArgumentList "cmd.exe /c $cmd" -Verb runAs

    } catch {
            [String]$cmd = "@echo off & chcp 850 >nul && cls && powershell.exe -Executionpolicy Unrestricted -File `"$PSCommandPath`" `"$ScriptArgs`""
            Write-Host "Script is restarting, wait..."
            Start-Process -FilePath "conhost.exe" -ArgumentList "cmd.exe /c $cmd" -Verb runAs
    }
    Exit 1
